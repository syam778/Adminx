...
const BASE_URL = "http://localhost:3000/api/store/verify";
...
const StoreVerify = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(res.data.message || "Store verified successfully");
      setFormData({
        username: "",
        storeId: "",
        address: "",
    ...
    <div className="store-verify-container">
      ...
      <form onSubmit={handleSubmit} className="store-verify-form">
        ...
        <button type="submit" disabled={loading}>
          {loading ? "Submitting..." : "Verify Store"}
        </button>
      </form>
...
const StoreVerify = () => {
  ...
  // Inline API call
  const verifyStoreData = async (data) => {
    return axios.post("http://localhost:3000/api/store/verify", data);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await verifyStoreData(formData);
      setMessage(res.data.message || "Store verified successfully");
    } catch (err) {
